...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid, trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None):
        ...
        self.R = R                  #  R: (3, 3)
        self.T = T                  #  T: (3, )
        self.FoVx = FoVx            #  float
        self.FoVy = FoVy            #  float
        self.image_name = image_name

        try:
            self.data_device = torch.device(data_device)    # 'cuda'
        except Exception as e:
            print(e)
            print(f"[Warning] Custom device {data_device} failed, fallback to default cuda device")
            self.data_device = torch.device("cuda")

        self.original_image = image.clamp(0.0, 1.0).to(self.data_device)    # [0, 1]: (3, 800, 800)
        self.fid = torch.Tensor(np.array([fid])).to(self.data_device)       # (1)
        self.image_width = self.original_image.shape[2]                     # 400
        self.image_height = self.original_image.shape[1]                    # 400
        self.depth = torch.Tensor(depth).to(self.data_device) if depth is not None else None    # None

        if gt_alpha_mask is not None:   # False
