...
def loadCam(args, id, cam_info, resolution_scale):
    orig_w, orig_h = cam_info.image.size        # 400, 400
    ...
    if args.resolution in [1, 2, 4, 8]:
        ...
        if args.resolution == -1:
            ...
            global_down = orig_w / args.resolution  # 1

        scale = float(global_down) * float(resolution_scale)    # 1
        resolution = (int(orig_w / scale), int(orig_h / scale)) # 400, 400

    resized_image_rgb = PILtoTorch(cam_info.image, resolution)

    gt_image = resized_image_rgb[:3, ...]   # (3, 400, 400)
    loaded_mask = None
    ...
    return Camera(
                    ...
                    R=cam_info.R,                     #  R: (3, 3)
                    T=cam_info.T,                     #  T: (3, )
                    FoVx=cam_info.FovX,               #  float
                    FoVy=cam_info.FovY,               #  float
                    image=gt_image,                   # (3, 400, 400)
                    gt_alpha_mask=loaded_mask,        # None
                    image_name=cam_info.image_name,   # String
                    uid=id,                           # Cameralist idx
                    data_device=args.data_device if not args.load2gpu_on_the_fly else 'cpu',  # 'cuda'
                    fid=cam_info.fid,                 #  float
                    depth=cam_info.depth              # None
                  )
